...
# This function handles both optional and required desktop applications
# Args:
...
# This ensures scripts calling install_optional_desktop_application still work
install_optional_desktop_application() {
    # Convert old parameter order (flatpak_id, app_name, apt_package)
    # to new order (app_name, flatpak_id, apt_package)
    local flatpak_id="$1"
    local app_name="$2"
    local apt_package="${3:-}"

    install_desktop_application "$app_name" "$flatpak_id" "$apt_package"
}
